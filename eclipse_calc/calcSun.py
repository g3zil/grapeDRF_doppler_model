...
calcAzZen                    calculate sun azimuth and zenith angle
...
def calcAzZen( t, localtime, latitude, longitude, zone ):
...
def calcTerminator( date, latitudes, longitudes,nlats=50,nlons=50 ):
    ...
    for ilat in range(1,nlats+1):
        for ilon in range(nlons):
            az,el = calcAzZen(t, ut, lats[-ilat], lons[ilon], 0.) 
            zen[-ilat,ilon] = el
        a = (90 - zen[-ilat,:])
...
def calculate_solarAzEl(date_time,lat,lon):
    """
    Calculate the solar aziumth and elevation angle for a list of dates.
        dates: datetime.datetime object or iterable of datetime.datetime objects
        lat:   latitude to calculated solar azimumth
        lon:   longitude to calculate solar azimuth

    Returns:
        az, el: Tuple of solar azimuths and elevations in degrees.

    Nathaniel A. Frissell - 3 June 2024
    """
    import numpy as np
    ...
    for date in date_time:
        ...
        t       = calcTimeJulianCent(jd)
        ut      = ( jd - (int(jd - 0.5) + 0.5) )*1440.
        az,zen  = calcAzZen(t, ut, lat, lon, 0.)
        el      = 90. - zen
        azs.append(az)
        els.append(el)
    return azs,els
